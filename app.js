...
const configuredSiteUrl = process.env.SITE_URL || process.env.PUBLIC_URL || "";
...
const SESSION_SECRET = process.env.SESSION_SECRET || (isProduction
    ...
    : "wanderly-local-development-secret-change-before-production");
// Railway exposes the attached volume mount path automatically at runtime.
// STORAGE_DIR remains supported for other hosts and can override it explicitly.
const configuredStorageDir = process.env.STORAGE_DIR || process.env.RAILWAY_VOLUME_MOUNT_PATH || "";
const storageDir = configuredStorageDir
    ? path.resolve(configuredStorageDir)
    : isProduction
        ? path.join(os.tmpdir(), "chinar-trails")
        : null;
if (isProduction) {
    if (configuredStorageDir && !path.isAbsolute(configuredStorageDir)) throw new Error("STORAGE_DIR or RAILWAY_VOLUME_MOUNT_PATH must be an absolute path on a writable persistent volume.");
    if (!process.env.SESSION_SECRET) console.warn("SESSION_SECRET is not set. A temporary secret was generated; sessions will be invalidated when the process restarts.");
    if (!configuredSiteUrl) console.warn("SITE_URL is not set. Canonical URLs will use the incoming request host; set SITE_URL or PUBLIC_URL for a fixed public URL.");
    if (!configuredStorageDir) console.warn(`STORAGE_DIR is not set. Using temporary storage at ${storageDir}; configure a persistent volume or external database to retain data across deploys.`);
}
// Hosting behind a TLS-terminating reverse proxy needs this for secure cookies.
...
app.use(
    session({
        ...
        store: isProduction ? new (require("./lib/file-session-store"))(path.join(storageDir, "sessions")) : undefined,
        resave: false,
...
app.use((req, res, next) => {
    ...
    res.locals.siteUrl = siteOrigin(req);
    next();
});

// Static files
const uploadsDir = storageDir ? path.join(storageDir, "uploads") : path.join(__dirname, "public", "uploads");
if (storageDir) {
    fs.mkdirSync(uploadsDir, { recursive: true });
    for (const filename of fs.readdirSync(path.join(__dirname, "public", "uploads"))) {
        const source = path.join(__dirname, "public", "uploads", filename);
        const target = path.join(uploadsDir, filename);
        if (fs.statSync(source).isFile() && !fs.existsSync(target)) fs.copyFileSync(source, target);
    }
    app.use("/uploads", express.static(uploadsDir), (req, res) => res.sendStatus(404));
}
...
const bundledContentFiles = new Set(["destinations.json", "itineraries.json", "hotels.json", "media.json", "testimonials.json"]);
...
function ensureDataFile(filePath, initialData = []) {
    ...
    if (!fs.existsSync(filePath)) {
        const bundledFile = path.join(__dirname, "data", path.basename(filePath));
        if (storageDir && bundledContentFiles.has(path.basename(filePath)) && fs.existsSync(bundledFile)) {
            fs.copyFileSync(bundledFile, filePath);
        } else {
    ...
    try {
        ...
        if (isProduction) throw new Error(`Cannot read ${filePath}: ${error.message}`);
        fs.writeFileSync(filePath, JSON.stringify(initialData, null, 2));
        return initialData;
    }
}
...
const initialAdminEmail = process.env.ADMIN_EMAIL || "admin@wanderly.com";
const initialAdminPassword = process.env.ADMIN_PASSWORD || "admin123";
const productionAdminConfigured = Boolean(process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD && process.env.ADMIN_PASSWORD.length >= 12);
if (isProduction && !productionAdminConfigured) {
    console.warn("No production admin account will be provisioned until ADMIN_EMAIL and ADMIN_PASSWORD (at least 12 characters) are configured.");
}
let users = ensureDataFile(usersFile, isProduction ? [] : [
    {
        id: 1,
        name: "Admin User",
        email: initialAdminEmail,
        password: bcrypt.hashSync(initialAdminPassword, 10),
        role: "admin"
    }
]);
if (isProduction) {
    const matchingAdmin = users.find(user => user.email.toLowerCase() === initialAdminEmail.toLowerCase() && user.role === "admin");
    if (productionAdminConfigured) {
        // Keep the configured production credentials authoritative. If there is
        // one existing admin and its email changed, rotate that account in place.
        const soleExistingAdmin = users.filter(user => user.role === "admin").length === 1
            ? users.find(user => user.role === "admin")
            : null;
        const adminToConfigure = matchingAdmin || soleExistingAdmin;
        if (adminToConfigure) {
            const credentialsChanged = adminToConfigure.email.toLowerCase() !== initialAdminEmail.toLowerCase()
                || !bcrypt.compareSync(initialAdminPassword, adminToConfigure.password);
            if (credentialsChanged) {
                adminToConfigure.email = initialAdminEmail;
                adminToConfigure.password = bcrypt.hashSync(initialAdminPassword, 10);
        ...
        const insecureDefaultAdmin = users.find(user => user.email === "admin@wanderly.com" && user.role === "admin" && bcrypt.compareSync("admin123", user.password));
        if (insecureDefaultAdmin) {
            users = users.filter(user => user !== insecureDefaultAdmin);
            saveData(usersFile, users);
        }
    }
}
    ...
    res.render("destination", {
        ...
        ...pageSeo(`${destination.name}, ${destination.country} | Chinar Trails`, destination.description, {
            ...
            structuredData: {
                ...
                image: `${siteOrigin(req)}${destination.image}`,
...
app.post("/admin/media/:id/delete", requireLogin, requireAdmin, requireAdminCsrf, (req, res) => {
    ...
    if (String(media.path).startsWith("/uploads/")) {
        const filePath = path.join(uploadsDir, path.basename(media.path));
...
app.get("/sitemap.xml", (req, res) => {
    ...
    const urls = [...staticPaths, ...dynamicPaths]
        .map(pathname => `  <url><loc>${siteOrigin(req)}${pathname}</loc></url>`)
        .join("\n");
    res.type("application/xml").send(`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>`);
});

// Railway uses this endpoint to confirm the app is ready before routing traffic.
...
app.listen(PORT, "0.0.0.0", () => {
    console.log(`Travel website listening on port ${PORT}`);
});
